...
namespace OpenFeature.Contrib.Providers.GOFeatureFlag.Test.v2.utils;
...
public class AssertUtil
{
    public static void JsonEqual(string expectedJson, string actualJson)
    {
        ...
        var expected = JsonSerializer.Deserialize<object>(expectedJson);
        var actual = JsonSerializer.Deserialize<object>(actualJson);

        if (!JsonSerializer.Serialize(expected).Equals(JsonSerializer.Serialize(actual)))
        {
