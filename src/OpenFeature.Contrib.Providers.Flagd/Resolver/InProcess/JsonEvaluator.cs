  ...
  internal class FlagConfiguration
    ...
    internal class JsonEvaluator
    {
        ...
        internal JsonEvaluator(string selector)
        {
            ...
            // TODO: rename to xxxRule?
            RuleRegistry.AddRule("starts_with", new StartsWith());
            RuleRegistry.AddRule("ends_with", new EndsWith());
            RuleRegistry.AddRule("sem_ver", new SemVerEvaluator());
            ...
            EvaluationContext context = null)
        {
            ...
            if (_flags.TryGetValue(flagKey, out var flagConfiguration))
            {
                    ...
                    flagConfiguration.Targeting.ToString() != "{}")
                {
                    ...
                    dict = dict.Add(FlagdProperties.FlagdPropertiesKey, new Value(new Structure(flagdProperties)));
                    // TODO: all missing comments
                    var targetingString = flagConfiguration.Targeting.ToString();

                    var rule = JsonNode.Parse(targetingString);

                    var contextObj = JsonNode.Parse(JsonSerializer.Serialize(ConvertToDynamicObject(dict)));
        ...
        static Value ConvertJObjectToOpenFeatureValue(JsonObject jsonValue)
        {
            ...
            foreach (var property in jsonValue.AsEnumerable())
            {
                switch (property.Value.GetValueKind())
                {
                    ...
                    case JsonValueKind.Number:
                        // TODO: add float
                        result.Add(property.Key, new Value((Int64)property.Value));
                        break;
                    ...
                    case JsonValueKind.Object: case JsonValueKind.Array:
                        result.Add(property.Key, ConvertJObjectToOpenFeatureValue(property.Value.AsObject()));
                        break;
