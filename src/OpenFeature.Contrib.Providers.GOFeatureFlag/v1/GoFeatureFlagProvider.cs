...
using OpenFeature.Contrib.Providers.GOFeatureFlag.v1.extensions;
using OpenFeature.Contrib.Providers.GOFeatureFlag.v1.hooks;
using OpenFeature.Contrib.Providers.GOFeatureFlag.v1.models;
using OpenFeature.Model;

namespace OpenFeature.Contrib.Providers.GOFeatureFlag.v1;
...
public class GoFeatureFlagProvider : FeatureProvider
{
    ...
    public override IImmutableList<Hook> GetProviderHooks()
    {
        ...
        hooks.Add(new EnrichEvaluationContextHook(this._exporterMetadata));
    ...
    {
        this._exporterMetadata = options.ExporterMetadata ?? new ExporterMetadata();
        this._exporterMetadata.Add("provider", ".NET");
        this._exporterMetadata.Add("openfeature", true);
        ...
        this._httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue(ApplicationJson));
        ...
        EvaluationContext context = null, CancellationToken cancellationToken = default)
    {
        try
        {
            var resp = await this.CallApi(flagKey, defaultValue, context).ConfigureAwait(false);
        ...
        EvaluationContext context = null, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                var value = this.ConvertValue((JsonElement)resp.Value);
        ...
        EvaluationContext context = null)
    {
        ...
        var response = await this._httpClient.PostAsync($"ofrep/v1/evaluate/flags/{flagKey}",
    ...
    private Value ConvertValue(JsonElement value)
    {
        ...
        if (value.ValueKind == JsonValueKind.Object)
        {
            ...
            while (objEnumerator.MoveNext())
            {
                ...
                var currentValue = this.ConvertValue(current.Value);
                if (currentValue != null)
                {
                    dict.Add(current.Name, this.ConvertValue(current.Value));
                }
            }
        ...
        if (value.ValueKind == JsonValueKind.Array)
        {
            ...
            while (arrayEnumerator.MoveNext())
            {
                ...
                var convertedValue = this.ConvertValue(current);
